# ERP LONGTECH System Architecture / Kiến trúc hệ thống ERP LONGTECH

This directory contains comprehensive architecture documentation for ERP LONGTECH, including system diagrams, data flow, deployment topology, and event-driven patterns.

Thư mục này chứa tài liệu kiến trúc toàn diện cho ERP LONGTECH, bao gồm sơ đồ hệ thống, luồng dữ liệu, cấu trúc triển khai và các mẫu hướng sự kiện.

---

## System Overview / Tổng quan hệ thống

The ERP LONGTECH system is built on a modern, cloud-native microservices architecture with the following key layers:

Hệ thống ERP LONGTECH được xây dựng dựa trên kiến trúc microservices hiện đại, hướng tới đám mây với các lớp chính sau:
...
ERP LONGTECH consists of 14+ services and applications with carefully managed dependencies:

ERP LONGTECH bao gồm 14+ dịch vụ và ứng dụng với các phụ thuộc được quản lý cẩn thận:
...
ERP LONGTECH is deployed to Kubernetes clusters managed by Terraform:

ERP LONGTECH được triển khai cho các cụm Kubernetes được quản lý bởi Terraform:
...
ERP LONGTECH uses NATS JetStream for event streaming between microservices:

ERP LONGTECH sử dụng NATS JetStream để phát trực tuyến sự kiện giữa các microservices:
...
- [API Documentation](./API.md) _(coming soon)_
- [Event Catalog](./EVENTS.md) _(coming soon)_
- [Monitoring & Observability](./OBSERVABILITY.md) _(coming soon)_

---

**Last Updated / Cập nhật cuối**: 2026-03-29
**Maintained by / Được duy trì bởi**: ERP LONGTECH Team (based on VietERP)
**Contact / Liên hệ**: architecture@longtech.vn
